...
class T_TestTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, x, y):
        ...
        pos_x = x[mask]
        neg_x = x[~mask]
        print(f" Postive Size: {pos_x.shape[0]}")
        print(f"Negative Size: {neg_x.shape[0]}")
...
# Spare matrix of shape (# train rows, vectorizer embedding size)
start_time = time.time()
pipeline_predict.fit(train_x, train_y)
print(f"Prediction pipeline fitting: {(time.time() - start_time):.1f} s")

start_time = time.time()
preds = pipeline_predict.predict_proba(test_x)
print(f"Test set AUC Score: {roc_auc_score(test_y, y_score=preds[:, 1]):.4f}")
print(f"Prediction pipeline prediction: {(time.time() - start_time):.1f} s")